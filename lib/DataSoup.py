...
class Node():
    ...
    def getDataTag( self, data, tag_name ):
    ...
    def getDataBool( self, data, tag_name ):
...
class Company( Node ):
    ...
    def __repr__( self ):
        ''' representation '''
        return '<Company %s (%s)>' % ( self.name, self.wp_cpt_id )
...
class Builder( Node ):
    ...
    def __init__( self, data, company, clientSlugs ):
        ''' constructor '''
        # relationships
        self.subdivs = []
        self.plans = []
    ...
    def addRelationship( self, key, value ):
        ''' add relationship '''
        if value != -1:
...
class Subdivision( Node ):
    ...
    def __init__( self, data, company, builder, clientSlugs, filterList ):
        ''' constructor '''
        # relationships
        self.builder = []
        self.plans = []
        ...
        self.slug = UT.slugify( '%s %s' % (self.id, self.name) )
        self.wp_cpt_id = UT.BDXgetMatchingWPID( self.slug, clientSlugs )
        self.status = data.attrs['Status'] or False
        self.style = Node.getDataTag( self, data, 'CommunityStyle' ) or ' '
        self.link_site = Node.getDataTag( self, data, 'SubWebsite' ) or ' '
        self.link_design_center = Node.getDataTag( self, data, 'EnvisionDesignCenter' ) or ' '
        self.link_video_tour = Node.getDataTag( self, data, 'SubVideoTour' ) or ' '
        self.price_low = data.attrs['PriceLow'] or False
        self.price_high = data.attrs['PriceHigh'] or False
        self.size_low = data.attrs['SqftLow'] or False
        self.size_high = data.attrs['SqftHigh'] or False
        ...
        self.headline = Node.getDataTag( self, data, 'MarketingHeadline' ) or 'Welcome to %s' % (self.name)
    ...
    def formatAddress( self, data ):
        ''' format subdiv address '''
        address = data.findChildren('SubAddress', recursive=False) or False
        data_out = {}
        for elm in address:
            ...
            country = Node.getDataTag( self, data, 'SubCountry' )
    ...
    def formatOffice( self, data ):
        ''' format office data '''
        office_data = data.findChildren('SalesOffice', recursive=False) or False
        for office in office_data:
            sales_agents = office.findChildren('Agent') or False
            sales_hours = office.find('Hours') or False
            phone_raw = office.findChildren('Phone') or False
            address = office.findChildren('Address') or False
            for elm in address:
                ...
                country = Node.getDataTag( self, data, 'Country' )
            ...
            if sales_agents:
                ...
                self.agents.append('')
            self.agents = ', '.join( self.agents ) or ' '
            # format phone number
            self.office_phone = ''
            if phone_raw:
    ...
    def formatSchools( self, data ):
        ''' format school data '''
        school_data = data.findChildren('Schools', recursive=False) or ''
        self.schools = []
        # loop school district locations
        if len(school_data) > 0:
            for school in school_data:
                data_obj = {}
                district = school.find('DistrictName').text.strip() if not school.find('DistrictName') == None else ''
                elementary = school.findChildren('Elementary') or ''
                middle = school.findChildren('Middle') or ''
                high = school.findChildren('High') or ''
    ...
    def formatSubdivImages( self, data, builder ):
...
class Plan( Node ):
    ...
    def __init__( self, data, company, builder, subdiv, clientSlugs, filterList ):
        ''' constructor '''
        # relationships
        self.builder = []
        self.subdiv = []
        ...
        self.base_price = str( int( math.floor( int( self.actual_price ) / 100000.0 ) ) * 100000 )
        ...
        self.num_baths = str( int(num_full_baths) + (int(num_half_baths) / 2) ).replace('.0', '')
    ...
    def formatLivingAreas( self, data ):
        ...
        for space in areas:
            ...
            s_name = space.name
            s_val = space.text
            all_spaces[s_type] = all_spaces[s_type] + 1 if all_spaces.get(s_type) else int(s_val)
        return all_spaces

    def formatAmenities( self, data ):
        ...
        for feature in amenities:
            ...
            a_name = feature.name
            a_val = feature.text
            all_feat[a_type] = all_feat[a_type] + 1 if all_feat.get(a_type) else int(a_val)
        return all_feat

    def formatPlanImages( self, data ):
        ''' format plan images '''
        self._elevations = self._floorplans = self._interiors = []
...
class BDXDataSoup():
    ...
    def __repr__( self ):
        ''' representation '''
        return '<DataSoup %s (%s) compiled: %s>' % ( self.client.CLIENT_NAME, self.cid, self.date )

    def __init__( self, data_file, client ):
        ''' constructor '''
        self.raw = { 'builders': [], 'subdivs': [], 'plans': [] }
        self.json = { 'builders': [], 'subdivs': [], 'plans': [] }
        self.client = client
        ...
        for company in companies:
            ...
            for builder in builders:
                ...
                for subdiv in subdivs:
                    # Subdivision data
                    this_subdiv = Subdivision( subdiv,
                        company=this_company,
                    ...
                    # Data relationships
                    this_builder.addRelationship( 'subdivs', this_subdiv.wp_cpt_id )	# Builder Relationship
                    this_subdiv.addRelationship( 'builder', this_builder.wp_cpt_id )	# Subdiv Relationship
                    # Plan xml
                    ...
                    for plan in plans:
                        ...
                        # DEBUG BLOCK
                        # print( this_builder.name )
                        # print( this_builder.wp_cpt_id )
                        # print( this_subdiv.name )
                        # print( this_subdiv.wp_cpt_id )
                        # print( this_plan.name )
                        # print( this_plan.wp_cpt_id )
                        # print( f'$ {this_plan.actual_price} ({this_plan.base_price})' )
                        # print()

                        # Data relationships
                        this_builder.addRelationship( 'plans', this_plan.wp_cpt_id )	# Builder-Plan Relationship
                        this_subdiv.addRelationship( 'plans', this_plan.wp_cpt_id )		# Subdiv-Plan Relationship
                        this_plan.addRelationship( 'builder', this_builder.wp_cpt_id )	# Plan-Builder Relationship
                        this_plan.addRelationship( 'subdiv', this_subdiv.wp_cpt_id )	# Plan-Subdiv Relationship
                        # add PLAN to dataset
