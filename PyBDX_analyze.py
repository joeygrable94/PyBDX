...
CLIENT = get_client()
BDX: PyBDX = PyBDX(
    client=CLIENT, download=False, analyze=False, convert=False, upload=False
)
...
# rebuild datasoup
BDX_curr: BDXDataSoup | None = BDX.ReheatSoup(curr_file, CLIENT)
...
for i, curr_plan in enumerate(BDX_curr.raw["plans"]):
    ...
    cp_builder = (
        BDX.findMatchingCPT(
            needle=curr_plan.builder[0], haystack=BDX_curr.raw["builders"]
        )
        if len(curr_plan.builder) > 0
        else None
    )
    cp_subdiv = (
        BDX.findMatchingCPT(
            needle=curr_plan.subdiv[0], haystack=BDX_curr.raw["subdivs"]
        )
        if len(curr_plan.subdiv) > 0
        else None
    )

    # previous plan, builder, and subdiv
    prev_plan = (
        BDX.findMatchingPlan(needle=curr_plan, haystack=BDX_prev.raw["plans"]) or None
    )
    pp_builder = (
